...
# ------------------------------------------------------------
SESSION_INFO_R = r"""
# If script is being executed directly, set options and re-source to get line numbers
.Last <- function() {
    ...
    tryCatch({
        ...
        if (exists(".runinfo.session.file.orig")) {
            system2(
                "cloudsh",
                c("mv", .runinfo.session.file, .runinfo.session.file.orig)
            )
        }
    }, error = function(e) {
        # Ignore errors in .Last to avoid masking original errors
        cat(
            "Warning: Failed to write session info to ",
            .runinfo.session.file,
            ": ",
            conditionMessage(e),
            "\n",
...
if (!interactive() && sys.nframe() == 0L) {
    # Set options and re-source ourselves to get line numbers
    options(keep.source = TRUE, rlang_trace_format_srcrefs = TRUE)
    if (!requireNamespace("rlang", quietly = TRUE)) {
        options(error = function() { traceback(3); quit(status = 1) })
    } else {
        options(error = quote({ rlang::entrace(); quit(status = 1) }))
    }
